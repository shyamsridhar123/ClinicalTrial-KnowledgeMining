...
CREATE INDEX idx_documents_nct_id ON documents(nct_id);
...
CREATE TABLE IF NOT EXISTS chunks (
    ...
    page_start INTEGER,
    page_end INTEGER,
    embedding vector(512), -- BiomedCLIP embeddings
    created_at TIMESTAMP DEFAULT NOW()
);
...
CREATE INDEX idx_entities_normalized ON entities(normalized_id, normalized_source);
...
CREATE INDEX idx_relations_chunk_id ON relations(chunk_id);
...
CREATE UNIQUE INDEX idx_repo_nodes_external ON repo_nodes(external_id, source);
CREATE INDEX idx_repo_nodes_source ON repo_nodes(source);
CREATE INDEX idx_repo_nodes_term ON repo_nodes USING gin(to_tsvector('english', preferred_term));
...
CREATE INDEX idx_repo_edges_source ON repo_edges(source_node_id);
CREATE INDEX idx_repo_edges_target ON repo_edges(target_node_id);
CREATE INDEX idx_repo_edges_type ON repo_edges(relationship_type);
...
CREATE INDEX idx_meta_graphs_chunk ON meta_graphs(chunk_id);
...
CREATE INDEX idx_tag_summaries_meta_graph ON tag_summaries(meta_graph_id);
CREATE INDEX idx_tag_summaries_layer ON tag_summaries(layer);
CREATE INDEX idx_tag_summaries_embedding ON tag_summaries USING hnsw (embedding vector_cosine_ops);
...
-- Function to sync PostgreSQL entities to AGE vertices
CREATE OR REPLACE FUNCTION sync_entity_to_age()
RETURNS TRIGGER AS $$
BEGIN
    -- Insert entity as AGE vertex
    PERFORM ag_catalog.cypher('clinical_kg', $$
        CREATE (e:Entity {
            id: $entity_id,
            text: $entity_text,
            type: $entity_type,
    ...
    $$, jsonb_build_object(
        ...
        'entity_text', NEW.entity_text,
        'entity_type', NEW.entity_type,
        'confidence', NEW.confidence,
        'normalized_id', COALESCE(NEW.normalized_id, '')
    ));
...
-- Function to sync PostgreSQL relations to AGE edges
CREATE OR REPLACE FUNCTION sync_relation_to_age()
RETURNS TRIGGER AS $$
BEGIN
    -- Create relationship edge in AGE
    PERFORM ag_catalog.cypher('clinical_kg', $$
    ...
    $$, jsonb_build_object(
        'subject_id', (SELECT e.id::text FROM entities e WHERE e.id = NEW.subject_entity_id),
        'object_id', (SELECT e.id::text FROM entities e WHERE e.id = NEW.object_entity_id),
        'predicate', NEW.predicate,
        'confidence', NEW.confidence,
        'evidence', NEW.evidence_span
    ));
...
-- Triggers to maintain AGE graph consistency
CREATE TRIGGER entities_to_age_trigger
    AFTER INSERT ON entities
    FOR EACH ROW
    EXECUTE FUNCTION sync_entity_to_age();

CREATE TRIGGER relations_to_age_trigger  
    AFTER INSERT ON relations
    FOR EACH ROW
    EXECUTE FUNCTION sync_relation_to_age();

-- Views for common queries
CREATE VIEW entity_relations AS
SELECT 
    e1.entity_text as subject_text,
    e1.entity_type as subject_type,
    r.predicate,
    e2.entity_text as object_text,
...
CREATE VIEW entity_repository_mappings AS
SELECT 
    e.entity_text,
    e.entity_type,
    rn.external_id,
    rn.source as vocab_source,
    rn.preferred_term,
    rn.definition,
    e.confidence as extraction_confidence
FROM entities e
JOIN repo_nodes rn ON e.normalized_id = rn.external_id 
    AND e.normalized_source = rn.source;
