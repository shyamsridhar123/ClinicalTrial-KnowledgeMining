...
def run_graph_stats():
    ...
    try:
        ...
        with conn.cursor(row_factory=dict_row) as cur:
            # Get entity counts
            cur.execute("SELECT COUNT(*) as total FROM docintel.entities")
            entities = cur.fetchone()['total']
            
            # Get relation counts
            cur.execute("SELECT COUNT(*) as total FROM docintel.entity_relations")
            relations = cur.fetchone()['total']
            
            # Get embedding counts
            cur.execute("SELECT COUNT(*) as total FROM docintel.embeddings")
            embeddings = cur.fetchone()['total']
            
            # Get NCT counts
            cur.execute("SELECT COUNT(DISTINCT nct_id) as total FROM docintel.embeddings")
            ncts = cur.fetchone()['total']
...
with gr.Blocks(title="DocIntel - Clinical Trial Knowledge Mining", theme=gr.themes.Soft()) as app:
    ...
    with gr.Row():
        ...
        with gr.Column(scale=1, min_width=200):
            ...
            search_btn = gr.Button("🔍 Semantic Search", variant="primary", size="lg")
            nav_buttons.append(search_btn)
            
            health_btn = gr.Button("📊 System Health", size="lg")
            nav_buttons.append(health_btn)
            
            ingest_btn = gr.Button("📥 Data Ingestion", size="lg")
            nav_buttons.append(ingest_btn)
            
            parse_btn = gr.Button("📄 Document Parsing", size="lg")
            nav_buttons.append(parse_btn)
            
            embed_btn = gr.Button("🧠 Generate Embeddings", size="lg")
            nav_buttons.append(embed_btn)
            
            stats_btn = gr.Button("📈 Graph Statistics", size="lg")
            nav_buttons.append(stats_btn)
            
            test_btn = gr.Button("🧪 Context Tests", size="lg")
            nav_buttons.append(test_btn)
        
        # Main content area
        with gr.Column(scale=4):
            # Tab 1: Semantic Search (default/first)
            with gr.Group(visible=True) as search_section:
                gr.Markdown("## 🔍 Semantic Search")
                gr.Markdown("""
                Ask questions about clinical trials with context-aware Q&A.
                
                Automatic detection of negation, historical conditions, hypothetical scenarios, etc.
                """)
                ...
                search_query_btn = gr.Button("Search", variant="primary", size="lg")
